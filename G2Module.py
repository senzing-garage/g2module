...
class G2Module(object):
    ...
    G2_ENTITY_DEFAULT_FLAGS = (
        G2_ENTITY_INCLUDE_REPRESENTATIVE_FEATURES | G2_ENTITY_INCLUDE_ALL_RELATIONS
    )
    ...
    def init(self):
        ...
        retval = self._lib_handle.G2_init(p_module_name, p_ini_file_name, self._debug)
        ...
        if retval == -2:
            ...
            raise G2ModuleNotInitialized(
                "G2Module has not been successfully initialized"
            )
        elif retval < 0:
    ...
    def getExportFlagsForMaxMatchLevel(
        self, max_match_level, includeSingletons, includeExtraCols
    ):
        ...
        if max_match_level == 1:
            ...
            g2ExportFlags = (
                self.G2_EXPORT_INCLUDE_RESOLVED | self.G2_EXPORT_INCLUDE_POSSIBLY_SAME
            )
        elif max_match_level == 3:
            # Include possibly related relationships in addition to resolved entities & possibly same
            g2ExportFlags = (
                self.G2_EXPORT_INCLUDE_RESOLVED
                | self.G2_EXPORT_INCLUDE_POSSIBLY_SAME
                | self.G2_EXPORT_INCLUDE_POSSIBLY_RELATED
            )
            ...
            g2ExportFlags = (
                ...
                | self.G2_EXPORT_INCLUDE_POSSIBLY_SAME
                | self.G2_EXPORT_INCLUDE_POSSIBLY_RELATED
                | self.G2_EXPORT_INCLUDE_NAME_ONLY
            )
            ...
            g2ExportFlags = (
                ...
                | self.G2_EXPORT_INCLUDE_POSSIBLY_RELATED
                | self.G2_EXPORT_INCLUDE_NAME_ONLY
                | self.G2_EXPORT_INCLUDE_DISCLOSED
            )
        else:
    ...
    def getExportHandleFromFlags(self, exportType, g2ExportFlags, colNames=None):
        if exportType == "CSV":
            if colNames and isinstance(colNames, list):
                ...
                self._lib_handle.G2_exportCSVEntityReport_V2.argtypes = [
                    c_char_p,
                    c_int,
                ]
                exportHandle = self._lib_handle.G2_exportCSVEntityReport_V2(
                    colNames.encode(), g2ExportFlags
                )
            else:
    ...
    def fetchExportRecord(self, exportHandle):
        ...
        rowData = self._lib_handle.G2_fetchNext(
    ...
    def exportCSVEntityReport(
        self, max_match_level, g2ExportFlags, includeSingletons, includeExtraCols
    ):
        # type: (int, int) -> str
        ...
        resultString = b""
        fullG2ExportFlags_ = self.getExportFlagsForMaxMatchLevel(
            max_match_level, includeSingletons, includeExtraCols
        )
    ...
    def exportCSVEntityReportV2(
        ...
        exportHandle = self._lib_handle.G2_exportCSVEntityReport_V2(
            _csvColumnList, fullG2ExportFlags_
        )
        rowCount = 0
        resize_return_buffer(None, 65535)
        self._lib_handle.G2_fetchNext.argtypes = [c_void_p, c_char_p, c_size_t]
    ...
    def addRecord(self, dataSourceCode, recordId, jsonData, loadId=None):
        ...
        ret_code = self._lib_handle.G2_addRecord(
            _dataSourceCode, _recordId, _jsonData, _loadId
        )
        if ret_code == -2:
            self._lib_handle.G2_getLastException(tls_var.buf, sizeof(tls_var.buf))
            raise TranslateG2ModuleException(tls_var.buf.value)
    ...
    def searchByAttributes(self, jsonData):
        ...
        responseBuf = c_char_p(None)
        responseSize = c_size_t(0)
        self._lib_handle.G2_searchByAttributes.argtypes = [
            c_char_p,
            POINTER(c_char_p),
            POINTER(c_size_t),
            self._resize_func_def,
        ]
        ret_code = self._lib_handle.G2_searchByAttributes(
            _jsonData, pointer(responseBuf), pointer(responseSize), self._resize_func
        )
    ...
    def findPathByEntityID(self, startEntityID, endEntityID, maxDegree):
        ...
        self._lib_handle.G2_findPathByEntityID.argtypes = [
            c_longlong,
            c_longlong,
            c_int,
            POINTER(c_char_p),
        ...
        ret_code = self._lib_handle.G2_findPathByEntityID(
            startEntityID,
            endEntityID,
            maxDegree,
            pointer(responseBuf),
    ...
    def findNetworkByEntityID(self, entityList, maxDegree, buildOutDegree, maxEntities):
        ...
        self._lib_handle.G2_findNetworkByEntityID.argtypes = [
            c_char_p,
            c_int,
            c_int,
            c_int,
        ...
        ret_code = self._lib_handle.G2_findNetworkByEntityID(
            _entityList,
            maxDegree,
            buildOutDegree,
            maxEntities,
    ...
    def findNetworkByEntityIDV2(
        self, entityList, maxDegree, buildOutDegree, maxEntities, flags
    ):
        # type: (int) -> str
        """Find a network between entities in the system.
        Args:
    ...
    def findPathByRecordID(
        self, startDsrcCode, startRecordId, endDsrcCode, endRecordId, maxDegree
    ):
        # type: (str,str) -> str
        ...
        self._lib_handle.G2_findPathByRecordID.argtypes = [
            c_char_p,
            c_char_p,
            c_char_p,
            c_char_p,
        ...
        ret_code = self._lib_handle.G2_findPathByRecordID(
            _startDsrcCode,
            _startRecordId,
            _endDsrcCode,
            _endRecordId,
    ...
    def findPathByRecordIDV2(
        self, startDsrcCode, startRecordId, endDsrcCode, endRecordId, maxDegree, flags
    ):
        # type: (str,str) -> str
        """Find a path between two records in the system.
        Args:
    ...
    def findNetworkByRecordID(self, recordList, maxDegree, buildOutDegree, maxEntities):
        ...
        self._lib_handle.G2_findNetworkByRecordID.argtypes = [
            c_char_p,
            c_int,
            c_int,
            c_int,
        ...
        ret_code = self._lib_handle.G2_findNetworkByRecordID(
            _recordList,
            maxDegree,
            buildOutDegree,
            maxEntities,
    ...
    def findNetworkByRecordIDV2(
        self, recordList, maxDegree, buildOutDegree, maxEntities, flags
    ):
        # type: (str,str) -> str
        """Find a network between entities in the system.
        Args:
    ...
    def findPathExcludingByEntityID(
        self, startEntityID, endEntityID, maxDegree, excludedEntities, flags
    ):
        # type: (int) -> str
        """Find a path between two entities in the system.
        Args:
        ...
        self._lib_handle.G2_findPathExcludingByEntityID.argtypes = [
            c_longlong,
            c_longlong,
            c_int,
            c_char_p,
        ...
        ret_code = self._lib_handle.G2_findPathExcludingByEntityID(
            startEntityID,
            endEntityID,
            maxDegree,
            _excludedEntities,
    ...
    def findPathIncludingSourceByEntityID(
        ...
        self._lib_handle.G2_findPathIncludingSourceByEntityID.argtypes = [
            c_longlong,
            c_longlong,
            c_int,
            c_char_p,
        ...
        ret_code = self._lib_handle.G2_findPathIncludingSourceByEntityID(
            startEntityID,
            endEntityID,
            maxDegree,
            _excludedEntities,
    ...
    def findPathExcludingByRecordID(
        ...
        self._lib_handle.G2_findPathExcludingByRecordID.argtypes = [
            c_char_p,
            c_char_p,
            c_char_p,
            c_char_p,
        ...
        ret_code = self._lib_handle.G2_findPathExcludingByRecordID(
            _startDsrcCode,
            _startRecordId,
            _endDsrcCode,
            _endRecordId,
    ...
    def findPathIncludingSourceByRecordID(
        ...
        self._lib_handle.G2_findPathIncludingSourceByRecordID.argtypes = [
            c_char_p,
            c_char_p,
            c_char_p,
            c_char_p,
        ...
        ret_code = self._lib_handle.G2_findPathIncludingSourceByRecordID(
            _startDsrcCode,
            _startRecordId,
            _endDsrcCode,
            _endRecordId,
    ...
    def getEntityByEntityID(self, entityID):
        ...
        responseBuf = c_char_p(None)
        responseSize = c_size_t(0)
        self._lib_handle.G2_getEntityByEntityID.argtypes = [
            c_longlong,
            POINTER(c_char_p),
            POINTER(c_size_t),
            self._resize_func_def,
        ]
        ret_code = self._lib_handle.G2_getEntityByEntityID(
    ...
    def getEntityByRecordID(self, dsrcCode, recordId):
        ...
        responseBuf = c_char_p(None)
        responseSize = c_size_t(0)
        self._lib_handle.G2_getEntityByRecordID.argtypes = [
            c_char_p,
            c_char_p,
            POINTER(c_char_p),
            POINTER(c_size_t),
        ...
        ret_code = self._lib_handle.G2_getEntityByRecordID(
            _dsrcCode,
            _recordId,
            pointer(responseBuf),
            pointer(responseSize),
    ...
    def getRecord(self, dsrcCode, recordId):
        ...
        responseBuf = c_char_p(None)
        responseSize = c_size_t(0)
        self._lib_handle.G2_getRecord.argtypes = [
            c_char_p,
            c_char_p,
            POINTER(c_char_p),
            POINTER(c_size_t),
        ...
        ret_code = self._lib_handle.G2_getRecord(
            _dsrcCode,
            _recordId,
            pointer(responseBuf),
            pointer(responseSize),
